...
(function ($) {
        ...
        $(document).ready(function(){
          ...
          // BRAND SLIDER
            var swiper = new Swiper(".rts-brand__slider", {
             ...
             var swiper = new Swiper(".rts-brand__slider.style_two", {
                slidesPerView: 8,
                spaceBetween: 40,
                pagination: {
                  el: ".swiper-pagination",
    ...
    $(document).ready(function(){
        ...
        // pricing using switcher
        try {
  $('#mySwitch').on('click', function () {
    const isChecked = $(this).is(':checked');
    const tabId = isChecked ? 'yearly' : 'monthly';

    $(".price__content").removeClass("open").hide();
    $("#" + tabId).addClass("open").show();
  });

  // Default: Show Monthly tab
  $(".price__content").removeClass("open").hide();
  $("#monthly").addClass("open").show();
} catch (error) {
  console.error("Tab Button not enable this page", error);
}
    ...
    $(document).ready(function () {
      // currency conversion itself is handled by assets/js/currency/app.js
      // (rt-price / rt-currency-symbol); this only wires up the dropdown UI
