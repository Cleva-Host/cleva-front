let rtCurrencyConverter = {
    ...
    fetchAndStoreCurrency: function () {
        // free, keyless exchange-rate API (fawazahmed0/currency-api); the
        // fallback host is the same dataset served from a different CDN,
        // used if jsdelivr is unreachable
    ...
    getCurrencies: async function () {
        ...
        // fall back to an empty rate table (unconverted prices) if the fetch
        // above never populated localStorage, e.g. offline or blocked by CORS
    ...
    loadCurrencyConfig: async function () {
        fetch('assets/js/currency/configue.json')
            ...
            .then(data => {
                ...
                if (data.supportedCurrencies && selectElement) {
            ...
            const currencyCodeEl = document.querySelector('.easy-currency-switcher-toggle .currency-code');
            if (currencyCodeEl) {
                currencyCodeEl.innerHTML = userPreferedCurrency;
            }
    ...
    getCurrencyConfig: async function () {
        try {
            ...
            return { defaultCurrency: 'usd', currency_thousand_separator: ',', currency_decimal_separator: '.' };
        }
    },
    ...
    convertCurrency: async function (fromCurrency, toCurrency, value) {
        ...
        // Format the number with thousand grouping and the configured decimal separator
        let [integerPart, decimalPart] = convertedValue.split('.');
        integerPart = integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, currencyThousandSeparator);
        convertedValue = integerPart + currencyDecimalSeparator + decimalPart;
